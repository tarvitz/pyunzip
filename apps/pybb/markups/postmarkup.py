...
if six.PY3:
    from urllib.parse import urlparse, urlunparse, quote, unquote, quote_plus
elif six.PY2:
    try:
...
def annotate_link(domain):
    """
    This function is called by the url tag. Override to disable or
    change behaviour.
...
def strip_bbcode(bbcode):
    ...
    return u"".join([
        t[1] for t in PostMarkup.tokenize(bbcode)
        if t[0] == PostMarkup.TOKEN_TEXT
    ])
...
def create(include=None, exclude=None, use_pygments=True, **kwargs):
    ...
    include -- List or similar iterable containing the names
        of the tags to use
               If omitted, all tags will be used
    exclude -- List or similar iterable containing the names
        of the tags to exclude.
               If omitted, no tags will be excluded
    use_pygments -- If True, Pygments (http://pygments.org/) will
        be used for the code tag,
    ...
    if use_pygments:
        assert(pygments_available,
               "Install Pygments (http://pygments.org/) or call "
               "create with use_pygments=False")
...
def render_bbcode(bbcode, encoding="ascii", exclude_tags=None,
                  auto_urls=True):
    ...
    this encoding (defaults to 'ascii'). Ignore the encoding if
    you already have a unicode string
...
class TagBase(object):
    ...
    def __init__(self, name, enclosed=False, auto_close=False, inline=False,
                 strip_first_newline=False, **kwargs):
        """Base class for all tags.

        :param str name: The name of the bbcode tag
        :param bool enclosed: True if the contents of the tag should
            not be bbcode processed.
        :param bool auto_close: True if the tag is standalone and does
            not require a close tag.
        :param bool inline: True if the tag generates an inline html tag.
        """
    ...
    def get_contents_text(self, parser):
        """
        Returns the string between the the open and close tag,
        minus bbcode tags."""
        return u"".join(
...
class QuoteTag(TagBase):
    ...
    def render_open(self, parser, node_index):
        if self.params:
            return u'<blockquote><em>%s</em><br/>' % (
...
class _Parser(object):
    ...
    def get_text_nodes(self, node1, node2):
        ...
        return [
            node for node in self.nodes[node1:node2] if not callable(node)
        ]

    def begin_no_breaks(self):
        """Disables replacing of newlines with break tags at the
        start and end of text nodes.
        Can only be called from a tags 'open' method.
        """
        assert(
            self.phase == 1,
            "Can not be called from render_open or render_close"
        )
        self.no_breaks_count += 1

    def end_no_breaks(self):
        """
        Re-enables auto-replacing of newlines with break tags
        (see begin_no_breaks).
        """
        assert(
            self.phase == 1,
...
class PostMarkup(object):
    ...
    def tokenize(cls, post):
        ...
        while True:
            ...
            if brace_pos - pos > 0:
                yield (
                    PostMarkup.TOKEN_TEXT, post[pos:brace_pos], pos, brace_pos
                )
            ...
            if open_tag_pos != -1 and open_tag_pos < end_pos:
                yield (
                    PostMarkup.TOKEN_TEXT, post[pos:open_tag_pos],
                    pos, open_tag_pos
                )
                ...
                try:
                    ...
                    if post[end_pos] != '"':
                        ...
                        if end_pos == -1:
                            return
                        yield (
                            PostMarkup.TOKEN_TAG, post[pos:end_pos+1], pos,
                            end_pos + 1
                        )
                    else:
                        end_pos = find_first(post, end_pos, u'"]')
                        ...
                        if post[end_pos] == '"':
                            ...
                            if end_pos == -1:
                                return
                            yield (
                                PostMarkup.TOKEN_PTAG, post[pos:end_pos+1],
                                pos, end_pos + 1
                            )
                        else:
                            yield (
                                PostMarkup.TOKEN_TAG, post[pos:end_pos+1],
                                pos, end_pos
                            )
                    pos = end_pos+1
                except IndexError:
    ...
    def tagify_urls(self, postmarkup):
        ...
        text_tokens = []
        for tag_type, tag_token, start_pos, end_pos in self.tokenize(
                postmarkup
        ):
    ...
    def render_to_html(self,
        ...
        if not isinstance(post_markup, six.string_types):
            post_markup = force_text(post_markup, encoding, 'replace')
...
[b]Test[/b]"""
    ...
    tests.append("[code unknownlanguage]10 print "
                 "'In yr code'; 20 goto 10[/code]")

    tests.append("[url=http://www.google.com/coop/cse?"
                 "cx=006850030468302103399%3Amqxv78bdfdo]"
                 "CakePHP Google Groups[/url]")
    tests.append("[url=http://www.google.com/"
                 "search?hl=en&safe=off&client=opera&rls=en"
                 "&hs=pO1&q=python+bbcode&btnG=Search]"
                 "Search for Python BBCode[/url]")
    # Attempt to inject html in to unicode
    tests.append("[url=http://www.test.com/sfsdfsdf/ter?t=\">"
                 "</a><h1>HACK</h1><a>\"]Test Hack[/url]")

    tests.append('Nested urls, i.e. [url][url]www.becontrary.com[/url][/url], '
                 'are condensed in to a single tag.')
...
[/list]""")
    ...
    tests.append("[b]urls such as http://www.willmcgugan.com are "
                 "authomaticaly converted to links[/b]")
...
[/list]""")
    ...
    for test in tests:
        print(u"<pre>%s</pre>" % str(test.encode("ascii",
                                                 "xmlcharrefreplace")))
        print(u"<p>%s</p>" % str(
            post_markup(test).encode("ascii", "xmlcharrefreplace")))
        print(u"<hr/>\n")

    print(repr(post_markup('[url=<script>Attack</script>]Attack[/url]')))

    print(
        repr(post_markup('http://www.google.com/'
                         'search?as_q=bbcode&btnG='
        ...
        def testlinks(self):
            ...
            tests = [
                ...
                ('[link]http://www.willmcgugan.com[/link]',
                 '<a href="http://www.willmcgugan.com">'
                 'http://www.willmcgugan.com</a>')
            ]

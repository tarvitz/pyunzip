...
class WarningsMiddleware(object):
    """ Warnings Middleware, process user warnings and affect on user with
    his user warning level
    1: *
    2: **
    3: +
    4: ++
    ...
    def process_request(self, request):
        ...
        for w in warnings:
            if w.expired < datetime.now():
                ...
                w.user.save()
                #also we delete messages for the warning
            ...
            if w.level == 8 and not w.user.is_superuser:
                # ten years would be enough
                w.expired = datetime.now()+timedelta(days=365 * 10)
                w.user.is_active = False
                w.user.save()

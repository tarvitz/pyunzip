...
class PollForm(forms.ModelForm):
    vote = forms.ChoiceField(
        label=_("You option"), widget=forms.RadioSelect,
        required=True
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        attrs = {}
        """BoolField = forms.BooleanField
        BoolWidget = (
            forms.CheckboxInput if self.poll.is_multiple
            else forms.widgets.RadioInput
        )

        for item in self.poll.items.all():
            self.fields['item_%s' % item.pk] = BoolField(
                widget=BoolWidget(attrs=attrs),
                label=item.title, required=False
            )
        """
        items = self.poll.items.values('pk', 'title')
        choices = []
        for item in items:
            choices.append((item['pk'], item['title']))
        self.fields['vote'].choices = choices
        #if self.poll.is_multiple:
        #    self.fields['vote'].widget = forms.CheckboxInput(attrs=attrs)
